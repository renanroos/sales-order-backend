import { SalesOrderItemModel, SalesOrderItemPropsWithSnakeCaseProductId } from './sales-order-item';
...
export class SalesOrderHeaderModel {
    ...
    private validateItemsOnCreation(items: SalesOrderHeaderProps['items']): CreationPayloadValidationResult {
        if (!items || items?.length === 0) {
            return {
                hasError: true,
                error: new Error('A ordem de venda deve conter pelo menos um item')
            };
        }
